...
I've been messing around with gamedev on my M2 Mac and because I enjoy making things harder on myself apparently, 
I'm trying not to use Unity.

One of the goals for my fiddling was to be able to learn more about [Dear ImGui](https://github.com/ocornut/imgui),
an immediate-mode GUI library that's widely used across the industry for custom debug views and tooling. Getting ImGui
working with Monogame on my mac has been a fun adventure in building and maintaining native dependencies so I figured I'd write a bit about how I got it working.

## ImGui Native Dependencies
I made the decision to separate all of the native code work out from my game code and just copy over the built binaries
to a `lib` folder in that project.

In the native project, I made a fork of this supremely useful OSS project:
- [cimgui](https://github.com/AesteroidBlues/cimgui/): A generator to create a C wrapper around ImGui
...
- [ImGui.NET](https://github.com/AesteroidBlues/ImGui.NET/): A generator that creates C# bindings and interop automatically from cimgui
...
- [cimnodes](https://github.com/AesteroidBlues/cimnodes): A toolset for creating node-based visual scripts, similar to Unreal Blueprints
- [cimplot](https://github.com/AesteroidBlues/cimplot): A set of graphing and plotting tools for ImGui
- [cimguizmo](https://github.com/AesteroidBlues/cimguizmo): A library of 3d gizmos, also contains a sequencer and graph editor that are not parsed by the C generator.
...
The final hurdle came when I needed a bezier curve editor for my game's tooling. I found a great looking tool on [the ImGui discussion page](https://github.com/ocornut/imgui/issues/786#issue-172274967) but it requires ImGui calls that are only available in `imgui_internal.h` which is not exposed via ImGui.NET. Instead of trying to expose these calls to C# I decided it would be easier and more extensible to just build the widgets in C++ and invoke them from C#.

[Here](https://github.com/AesteroidBlues/startup-hell-native-dependencies/blob/master/src/ImGuiCurveWidget.h) I define a C-style header with the appropriate API flags to tell the compiler not to mangle the function names and include this header from a .cpp file that implements the widget. Then, using CMake, I can create a native binary that gets built in a manner similar to cimgui.
...
[Startup Hell Native Dependencies](https://github.com/AesteroidBlues/startup-hell-native-dependencies/)